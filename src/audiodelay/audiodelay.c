...
static void
gst_audio_delay_calculate_tick_time (GstBtAudioDelay * self)
{
  self->ticktime =
...
static void
gst_audio_delay_tempo_change_tempo (GstBtTempo * tempo, glong beats_per_minute,
...
static void
gst_audio_delay_tempo_interface_init (gpointer g_iface, gpointer iface_data)
{
...
static gboolean
gst_audio_delay_set_caps (GstBaseTransform * base, GstCaps * incaps,
    GstCaps * outcaps)
{
...
GType
gstbt_audio_delay_get_type (void)
{
  static GType type = 0;
  ...
  if (G_UNLIKELY (!type)) {
    ...
    type =
        g_type_register_static (GST_TYPE_BASE_TRANSFORM, "GstBtAudioDelay",
