/* $Id: e-gst-note2frequency.c,v 1.2 2006-01-07 17:33:28 ensonic Exp $ */
...
START_TEST(test_translate_base) {
  GstNote2Frequency *n2f;
  gdouble frq;
  ...
  frq=gst_note_2_frequency_translate(n2f,"A-3");
  //g_print("frq=%lf\n",frq);
...
START_TEST(test_translate_series) {
  GstNote2Frequency *n2f;
  ...
  while(notes[i]) {
	frq=gst_note_2_frequency_translate(n2f,notes[i]);
	//g_print("%s -> frq=%lf\n",notes[i],frq);
...
TCase *gst_buzztard_note2frequency_example_case(void) {
  ...
  tcase_add_test(tc,test_translate_base);
  tcase_add_test(tc,test_translate_series);
